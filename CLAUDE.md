...
**claude-in-chrome screenshots can lie about subtle canvas/color effects — and about ordinary DOM/CSS elements too** — this automation environment has been observed applying a forced-dark repaint to pages independent of their actual (correct) light/dark theme state: forcing `document.body`'s background/color inline with `!important` to known light-theme values and re-screenshotting still showed a dark page. Low-alpha canvas effects (ambient backgrounds, faint contour lines) can look completely absent in a screenshot while actually rendering correctly. Confirmed 2026-09-08 on a plain DOM element too, not just canvas: a screenshot of the live `/exams` "Lệnh Xuất Phát Mốc" modal appeared to be missing the spec's 3px `--ink` top bar (`.vtg-modal::before` in `exam-app/src/index.css`), but `getComputedStyle`/`getBoundingClientRect` in the same tab showed `document.documentElement.getAttribute('data-theme') === "light"`, `--paper: #F5F2EA` (cream), and the `::before` fully present (height 3px, background `--ink`) — the screenshot had force-repainted the whole page dark (including the dark-navy `--ink` bar against a now-dark modal), making a genuinely-present, spec-compliant element invisible. Before concluding *any* visual element (canvas or DOM) is broken/missing, sample it directly — `canvas.getContext('2d').getImageData(...)` for canvas, or `getComputedStyle(el)` / `getComputedStyle(el, '::before')` + `document.documentElement.getAttribute('data-theme')` for DOM/CSS — rather than trusting a screenshot alone. Cross-checking against a reference render (e.g. the same technique against the mockup's own canvas) is a good sanity check.
