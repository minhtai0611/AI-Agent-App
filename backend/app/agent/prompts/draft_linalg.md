...
{"available": true, "operation": "add"|"multiply"|"determinant"|"inverse"|"rank"|"rref"|"solve_system"|"lu"|"qr"|"cholesky", "matrices": [[[<row1>],[<row2>],...], ...]}
...
- `matrices` is a list of matrices (each a list of rows, each row a list of numbers).
  ...
  right-hand side).
- Never propose the "eigen" or "svd" operations — they are not offered through natural language.
