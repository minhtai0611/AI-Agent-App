...
export default function OracleBubble() {
  ...
  const { isOpen, open, close, pageContext, unreadCount, oracleStatus } = useOracle()
    ...
    <>
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            key="oracle-bubble"
            data-oracle-state={oracleStatus}
            style={{
              '--oracle-glow': glowColor + '40',
              background: 'linear-gradient(135deg, #1E2A44 0%, #0D1221 100%)',
