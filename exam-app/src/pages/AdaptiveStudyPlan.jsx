...
function MasteryBar({ score, stage }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="font-jakarta text-[0.625rem] min-w-[28px] text-right" style={{ color: color + 'CC' }}>
        {score}%
      </span>
    </div>
  )
    ...
    <div className="rounded-2xl border px-5 py-4 flex flex-col gap-3" style={{ background: bg, borderColor: border }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col gap-0">
            <span className="font-jakarta text-[0.6875rem] font-semibold" style={{ color }}>
              {plan.on_track ? '↗ Đúng hướng' : '⚠ Cần tăng tốc'}
            </span>
            <span className="font-jakarta text-[0.625rem] text-faint">dự kiến kỳ thi</span>
          </div>
        </div>
        <div className="flex flex-col items-end gap-0.5">
          <span className="font-fraunces text-[20px] font-bold text-foreground">{plan.solid_count}</span>
          <span className="font-jakarta text-[0.625rem] text-faint">/ {plan.total_concepts} vững</span>
        </div>
      </div>
      ...
      {plan.days_remaining != null && (
        <div className="flex items-center gap-2">
          ...
          <span className="font-jakarta text-[0.6875rem] text-faint">còn {plan.days_remaining} ngày</span>
        </div>
      )}

      <p className="font-jakarta text-xs leading-relaxed" style={{ color: color + 'CC' }}>
...
function FocusConceptCard({ concept }) {
  ...
  return (
    <div className="flex flex-col gap-2 px-3 py-2.5 rounded-xl border border-border bg-surface">
      <div className="flex items-start justify-between gap-2">
        <span className="font-jakarta text-xs font-semibold text-highlight leading-tight">
          {concept.name_vi}
        </span>
        <span className="font-jakarta text-[0.625rem] font-semibold px-1.5 py-0.5 rounded-md shrink-0"
      ...
      {concept.error_types.length > 0 && (
        <div className="flex gap-1 flex-wrap">
          {concept.error_types.map(t => (
            <span key={t} className="font-jakarta text-[0.625rem] text-destructive px-1.5 py-0.5 rounded bg-[#FB718514]">
              {t}
            </span>
          ))}
        </div>
...
function WeekSchedule({ week }) {
  ...
  return (
    ...
    >
      <button
        onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-5 py-4 hover:bg-surface-elevated transition"
      >
        <div className="flex items-center gap-3">
          <span className="font-fraunces text-sm font-bold text-foreground">Tuần {week.week}</span>
          <div className="flex gap-1.5">
            {week.focus_concepts.map(c => (
              <span key={c.concept_id} className="font-jakarta text-[0.6875rem] text-faint">
                {c.name_vi}
              </span>
            )).reduce((acc, el, i) => (i === 0 ? [el] : [...acc, <span key={`sep-${i}`} className="text-border">·</span>, el]), [])}
          </div>
        </div>
        <span className="font-jakarta text-xs text-faint">{open ? '▲' : '▼'}</span>
      </button>
          ...
          <div className="flex flex-col gap-1.5 pt-1">
            ...
            {week.daily_plan.map(({ day, items }) => (
              <div key={day} className="flex items-start gap-3 py-2 border-b border-border last:border-0">
                <span className="font-jakarta text-[0.6875rem] font-semibold text-faint w-14 shrink-0 pt-0.5">
                  {day}
                </span>
                <div className="flex flex-wrap gap-1.5">
                  {items.map((item, i) => {
                    if (item.type === 'sm2') {
                      return (
                        <span key={i} className="font-jakarta text-[0.6875rem] text-[#34D399] px-2 py-0.5 rounded-md bg-[#34D39914] border border-[#34D39930]">
                          📋 {item.label}
                        </span>
                      )
                    }
                    if (item.type === 'challenge') {
                      return (
                        <span key={i} className="font-jakarta text-[0.6875rem] text-primary px-2 py-0.5 rounded-md bg-[#F2A20C14] border border-[#F2A20C30]">
                          ⚡ {item.label}
                        </span>
                      )
                    }
                    return (
                      <span key={i} className="font-jakarta text-[0.6875rem] text-[#818CF8] px-2 py-0.5 rounded-md bg-[#6366F114] border border-[#6366F130]">
                        ✦ {item.name_vi}
                      </span>
                    )
                  })}
    ...
    >
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6 flex flex-col gap-6">
        {!user ? (
          ...
          <div className="flex flex-col items-center gap-3 py-16">
            <span className="font-jakarta text-[0.8125rem] text-destructive">{error}</span>
            <button
              ...
              className="px-4 py-2 rounded-xl font-jakarta text-xs font-semibold text-foreground border border-border hover:border-[#4A5A80] transition"
            >
              Thử lại
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-[#34D39930] bg-[#34D39908] font-jakarta text-xs font-semibold text-[#34D399] hover:bg-[#34D39914] transition"
              >
                📋 Ôn FSRS{plan.days_remaining != null ? '' : ''}
              </button>
              <button
                onClick={() => navigate('/practice/adaptive')}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-[#6366F130] bg-[#6366F108] font-jakarta text-xs font-semibold text-[#818CF8] hover:bg-[#6366F114] transition"
              >
            ...
            {/* No exam date nudge */}
            {plan.days_remaining == null && (
              <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-[#2A3A60] bg-surface-elevated">
                <span className="text-primary">📅</span>
                <div className="flex flex-col gap-0.5">
                  <span className="font-jakarta text-xs font-semibold text-foreground">
                    Thêm ngày thi để dự đoán điểm số
                  </span>
                  <button
                    onClick={() => navigate('/account')}
                    className="font-jakarta text-[0.6875rem] text-[#6366F1] hover:underline text-left transition"
                  >
            ...
            {plan.focus_concepts.length > 0 && (
              <div className="flex flex-col gap-3">
                <span className="font-fraunces text-sm font-bold text-foreground">
                  Ưu tiên luyện tập
                </span>
            ...
            {plan.focus_concepts.length === 0 && plan.weekly_schedule.length === 0 && (
              <div className="flex flex-col items-center gap-4 py-12 text-center">
                ...
                <p className="font-jakarta text-[0.8125rem] text-faint max-w-xs">
