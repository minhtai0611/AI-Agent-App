  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center gap-6 px-4 py-12"
      ...
      <div className="w-full max-w-sm rounded-2xl border border-primary/20 p-8 flex flex-col gap-5 text-center"
        ...
        <span className="font-sans text-[12px] text-dim uppercase tracking-widest">Thách đấu</span>
        ...
        <div className="flex flex-col items-center gap-1 py-4">
          <span className="font-sans text-[72px] font-bold text-primary leading-none">
            {challenger.score.toFixed(1)}
          </span>
          <span className="font-sans text-[16px] text-dim">/ 10</span>
        </div>
        <p className="font-sans text-[14px] text-muted">Bạn có thể vượt qua không?</p>
        <p className="font-sans text-[10px] text-dim">Không xác minh · Dữ liệu tự khai báo</p>
      </div>

      <button
        onClick={() => navigate(`/test/${challenger.examId}`, { state: { challengerScore: challenger.score, challengerName: challenger.name } })}
        className="px-10 py-3.5 rounded-xl bg-primary font-sans text-[15px] font-bold text-background hover:opacity-90 transition"
      >
        Nhận thách đấu →
      </button>
    </div>
