...
export default function ConceptMap() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-[#1E2A44]">
          ...
          {[0, 9, 10, 11, 12].map(g => (
            <button key={g}
              ...
              className={`px-3 py-1.5 rounded-full font-jakarta text-[11px] transition ${
                gradeFilter === g
                  ? 'bg-[#F2A20C] text-[#0A0E1A] font-bold'
                  : 'border border-[#1E2A44] text-[#64748B] hover:text-[#94A3B8]'
              }`}>
              {g === 0 ? 'Tất cả' : `Lớp ${g}`}
            </button>
          ))}
      ...
      {/* Stats bar */}
      <div className="flex items-center gap-6 px-6 py-2 bg-[#0D1221] border-b border-[#1E2A44]">
        <span className="font-jakarta text-[11px] text-[#475569]">{stats.total} khái niệm</span>
        <span className="font-jakarta text-[11px] text-[#60A5FA]">{stats.tried} đã học</span>
        <span className="font-jakarta text-[11px] text-[#22C55E]">{stats.strong} thành thạo ≥70%</span>
        <div className="ml-auto flex items-center gap-3">
          {[['#14532D','#22C55E','≥70%'],['#78350F','#F59E0B','40-69%'],['#7F1D1D','#EF4444','<40%'],['#1E2A44','#334155','Chưa học']].map(([bg,border,label]) => (
            <span key={label} className="flex items-center gap-1 font-jakarta text-[10px] text-[#64748B]">
      ...
      <div className="flex flex-1" style={{ height: 'calc(100vh - 120px)' }}>
        ...
        {/* Detail panel */}
        {selectedConcept && (
          <div className="w-72 border-l border-[#1E2A44] bg-[#0D1221] flex flex-col gap-4 p-5 overflow-y-auto">
            <div className="flex items-start justify-between">
              <div>
                <span className="font-fraunces text-[16px] font-bold text-[#F8FAFC]">{selectedConcept.name_vi}</span>
                <div className="font-jakarta text-[11px] text-[#64748B] mt-0.5">
            ...
            <button
              ...
              className="mt-auto w-full py-2.5 rounded-xl font-jakarta text-[13px] font-bold transition"
