...
export default function ErrorAnalysis() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A] pb-16">
      {/* Header */}
      <div className="flex items-center gap-4 px-6 py-4 border-b border-[#1E2A44]">
        ...
        {aiData?.cached && (
          <span className="px-2 py-0.5 rounded-full font-jakarta text-[10px] bg-[#1E2A44] text-[#475569]">cache 24h</span>
        )}
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pt-8 flex flex-col gap-8">
        {(!results || results.length < 3) ? (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="font-jakarta text-[13px] text-[#64748B] max-w-xs">Hoàn thành ít nhất 3 bài thi để xem phân tích lỗi sai.</p>
            <button onClick={() => navigate('/exams')}
              className="px-5 py-2.5 rounded-xl font-jakarta text-[13px] font-bold"
          ...
          <>
            ...
            <div className="bg-[#0D1221] border border-[#1E2A44] rounded-2xl p-6">
              ...
              <p className="font-jakarta text-[12px] text-[#64748B] mb-5">Hồ sơ loại lỗi của bạn từ toàn bộ lịch sử thi</p>
            ...
            <div className="bg-[#0D1221] border border-[#1E2A44] rounded-2xl p-6">
              ...
              <p className="font-jakarta text-[12px] text-[#64748B] mb-5">
                Trọng số tính theo độ gần đây (lỗi gần đây nặng hơn)
              </p>
              {barData.length === 0 ? (
                <p className="font-jakarta text-[13px] text-[#475569] text-center py-8">Chưa có dữ liệu lỗi.</p>
              ) : (
            ...
            <div className="bg-[#0D1221] border border-[#1E2A44] rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="font-jakarta text-[12px] text-[#64748B] mt-0.5">Top 3 hiểu lầm cốt lõi · ⚡2 Tia</p>
                </div>
                {!hasMisconceptions && !aiLoading && (
                  <button onClick={fetchAI}
                    className="px-4 py-2 rounded-lg font-jakarta text-[12px] font-bold transition"
                    style={{ background: '#F2A20C', color: '#0A0E1A' }}>
                    Phân tích ngay
                  </button>
                )}
              ...
              {aiLoading && (
                <div className="flex items-center gap-2 py-6">
                  ...
                  <span className="font-jakarta text-[13px] text-[#64748B]">AI đang phân tích lỗi sai của bạn...</span>
                </div>
              )}
              ...
              {hasMisconceptions && (
                <div className="flex flex-col gap-4">
                  {aiData.misconceptions.map((m, i) => (
                    <div key={i} className="flex gap-4 p-4 rounded-xl bg-[#111827] border border-[#1E2A44]">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <p className="font-jakarta text-[12px] text-[#64748B]">💡 {m.suggestion}</p>
                      </div>
                    </div>
                  ))}
                </div>
