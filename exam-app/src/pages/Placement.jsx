  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#0A0E1A] flex items-center justify-center">
        <span className="font-jakarta text-[14px] text-[#475569]">Đang tải câu hỏi...</span>
      </div>
    )
  }
        ...
        <div className="w-full max-w-md bg-[#0D1527] border border-[#1E2A44] rounded-2xl p-8 flex flex-col items-center gap-5 text-center">
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="font-jakarta text-[14px] text-[#94A3B8]">
              Bạn trả lời đúng <span className="text-[#F2A20C] font-semibold">{correct}/{answers.length}</span> câu
            </p>
          </div>
          ...
          <div className="flex gap-3 w-full">
            <button
              onClick={() => navigate('/')}
              className="flex-1 py-3 rounded-xl font-jakarta text-[14px] font-semibold bg-[#6366F1] text-white hover:bg-[#4F46E5] transition"
            >
              Bắt đầu học →
            </button>
          </div>
      ...
      <div className="w-full max-w-xl flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-0.5">
            <span className="font-jakarta text-[11px] font-semibold text-[#6366F1] tracking-[2px] uppercase">Kiểm tra năng lực</span>
            <span className="font-jakarta text-[13px] text-[#475569]">Câu {current + 1} / {questions.length}</span>
          </div>
          <button
            onClick={() => navigate('/')}
            ...
            {revealed && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }} className="flex flex-col gap-3">
                ...
                <button
                  onClick={handleNext}
                  disabled={submitting}
                  className="w-full py-3 rounded-xl font-jakarta text-[14px] font-semibold bg-[#6366F1] text-white hover:bg-[#4F46E5] transition disabled:opacity-60"
                >
