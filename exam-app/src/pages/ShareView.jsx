...
export default function ShareView() {
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-4 px-4">
        ...
        <p className="font-jakarta text-[0.8125rem] text-dim text-center">Link chia sẻ đã hết hạn hoặc bị thay đổi.</p>
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center gap-6 px-4 py-12"
      ...
      {/* Card */}
      <div className="w-full max-w-sm rounded-2xl border border-border p-8 flex flex-col gap-5"
        style={{ background: 'linear-gradient(135deg, #0D1221 0%, #0A0E1A 100%)' }}>

        <div className="flex items-center justify-between">
          <span className="font-jakarta text-xs text-dim">Kết quả thi thử</span>
          <span className="font-jakarta text-xs text-dim">{date}</span>
        </div>

        <p className="font-jakarta text-sm font-semibold text-muted leading-snug">{data.e}</p>
        ...
        <div className="flex justify-around border-t border-border pt-4">
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="font-jakarta text-[0.6875rem] text-faint">Câu đúng</span>
          </div>
          <div className="flex flex-col items-center gap-1">
        ...
        {topBadge && (
          <div className="flex items-center justify-center gap-1.5">
            ...
            <span className="font-jakarta text-xs text-muted">{topBadge.label}</span>
          </div>
        )}
        <p className="font-jakarta text-[0.625rem] text-[#2A3A50] text-center">✦ exam-app-ey0.pages.dev · Không xác minh</p>
      </div>

      {/* CTA */}
      <div className="flex flex-col items-center gap-3 text-center">
        <p className="font-jakarta text-[0.8125rem] text-dim">Bạn có thể đạt điểm cao hơn không?</p>
        <button
          onClick={() => navigate('/exams')}
          className="px-8 py-3 rounded-xl bg-primary font-jakarta text-sm font-bold text-primary-fg hover:opacity-90 transition"
        >
          Thi thử ngay →
        </button>
      </div>
